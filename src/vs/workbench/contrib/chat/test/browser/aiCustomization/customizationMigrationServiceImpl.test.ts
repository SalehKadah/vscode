...
suite('CustomizationMigrationService', () => {
	...
	test('computes file and MCP migration candidates for Agent Host sessions', async () => {
		...
		assert.deepStrictEqual({
			...
			hint: {
				message: 'Found 1 workspace and 2 user customizations that are present but not used by Copilot and could be migrated. Found 1 MCP server that is not fully supported by Copilot.',
